...
#define SUPER_ITEM_KEY 210 // $ 아이템(슈퍼모드) 색상
...
int game_time = 0;
bool is_day = true;
bool is_super_mode = false; // 슈퍼 모드 상태
int super_zombie_spawned = 0; // 슈퍼모드에서 소환된 좀비 수
int super_zombie_total = 0; // 슈퍼모드 좀비 총 수
...
int main() {
    ...
    init_pair(SUPER_ITEM_KEY, 10, COLOR_BLACK); // 진한 초록색 글자, 검정 배경 (10번은 ncurses에서 진한 초록)
    ...
    for (int i = 0; i < MAP_HEIGHT; i++) {
        ...
        for (int j = 0; j < MAP_WIDTH; j++) {
			...
			switch(c) {
				...
				case '$' : map[i][j] = '$' | COLOR_PAIR(SUPER_ITEM_KEY) | A_BOLD; break;
				default:
...
int grab_item() { //아이템 랜덤 줍기
    ...
    for (int dir=0;dir<4;++dir) { //경계값 검사는 안해도 된다. 어차피 끝부분은 벽이다.
        ...
        if (cur == '!') {
            ...
            int all_vac = 1;
            for (int i=0;i<5;++i) {
                if (!item.vac[i]) all_vac = 0;
            }
            if (!all_vac) return 0;
            // 슈퍼모드 진입: 좀비 모두 제거, 100마리 소환, 플레이어 강화
            // 1. 기존 좀비 모두 제거
            for (int i=0;i<zombie_count;++i) {
                zombies[i].alive = 0;
                map[zombies[i].point.y][zombies[i].point.x] = GROUND | COLOR_PAIR(GROUND_KEY);
            }
            for (int i=0;i<fzombie_count;++i) {
                fzombies[i].alive = 0;
                map[fzombies[i].point.y][fzombies[i].point.x] = GROUND | COLOR_PAIR(GROUND_KEY);
            }
...
void use_item(int item_num) {
    if(item_num == 'j') {
                    ...
                    for(int j = 0; j < zombie_count; ++j) {
                        if(zombies[j].alive && (zombies[j].point.y == ny) && (zombies[j].point.x == nx)) {
                            ...
                            if(zombies[j].hp <= 0) {
								zombies[j].alive = 0;
								is_alive = false;
								if(is_super_mode) super_zombie_spawned--;
							}
							break;
                        }
                    }
        ...
        if(item.packet >= 1) {
            --item.packet;
            if(is_super_mode) player.hp += 100;
            else player.hp += 10;
            // 만약 최대 HP가 100/1000이라면
            if(is_super_mode && player.hp > 1000) player.hp = 1000;
