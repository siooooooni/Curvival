...
UI_item item = {.gun = 30, .vac = {0}, .injection = 10, .packet = 10,.UI_hp = 100};
...
yx vaccine_coords[5]; // 백신 좌표 저장 (디버그용)
...
int main() {
	...
	while(1){
		...
		mvprintw(32, 0, "Vaccine(DEBUG): D(%d,%d) E(%d,%d) B(%d,%d) U(%d,%d) G(%d,%d)", vaccine_coords[0].y, vaccine_coords[0].x, vaccine_coords[1].y, vaccine_coords[1].x, vaccine_coords[2].y, vaccine_coords[2].x, vaccine_coords[3].y, vaccine_coords[3].x, vaccine_coords[4].y, vaccine_coords[4].x);
...
void make_vaccine(chtype map[MAP_HEIGHT][MAP_WIDTH],char a) {
    ...
    map[y][x] = a | COLOR_PAIR(6) | A_BOLD;
    if (a == 'D') vaccine_coords[0] = (yx){y, x};
    else if (a == 'E') vaccine_coords[1] = (yx){y, x};
    else if (a == 'B') vaccine_coords[2] = (yx){y, x};
    else if (a == 'U') vaccine_coords[3] = (yx){y, x};
    else if (a == 'G') vaccine_coords[4] = (yx){y, x};
}
